...
} from "../features/dashboard/lib/api.js";
import {
  ingestCases,
  buildIngestPayload,
} from "../features/cases/lib/casesApi.js";
...
export default function DashboardRoute() {
  ...
  const handleUpload = async (file) => {
    ...
    try {
      ...
      setProgress(100);
      try {
        const { summary, flagged } = buildIngestPayload(data, file.name);
        if (flagged.length) ingestCases(summary, flagged).catch(() => {});
      } catch {
        /* ignore */
      }
  ...
  const stats = useMemo(() => {
    ...
    const critical =
      analysis?.high_risk_alerts ??
      results.filter((r) => r["Risk Score"] >= 80).length;
    const avgRisk =
      analysis?.average_risk_score ??
      (results.length
        ? results.reduce((s, r) => s + r["Risk Score"], 0) / results.length
        : 0);
  ...
  const alertCount = results.filter(
    (r) => riskTier(r["Risk Score"]) === "critical",
  ).length;
