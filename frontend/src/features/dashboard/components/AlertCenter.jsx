...
const ICON = { critical: ShieldAlert, suspicious: AlertTriangle, watch: Activity }
  ...
  return (
    <section className="card p-6 h-full flex flex-col">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="font-display font-bold text-xl tracking-tight text-content-hi">Alert Center</h2>
          <p className="text-sm text-content-md">Live stream of suspicious accounts</p>
        </div>
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between gap-2">
                  <span className={`text-sm font-medium ${s.text}`}>{a.Alert}</span>
                  <span className="text-[11px] text-content-lo num shrink-0">{formatTime(i * 7)}</span>
                </div>
                <p className="mt-0.5 text-xs text-content-md">
                  Account <span className="num text-content-hi">#{String(a.Account).padStart(5, '0')}</span>
                  {' · '}Risk <span className="num" style={{ color: s.color }}>{a['Risk Score'].toFixed(1)}%</span>
                </p>
              </div>
            </motion.div>
          )
