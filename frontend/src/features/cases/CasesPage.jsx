...
import {
  ShieldCheck, ArrowLeft, RefreshCw, Snowflake, PauseCircle,
  Send, CheckCircle2, Clock, FileText, X, AlertTriangle,
} from 'lucide-react'
import {
  listCases, getCase, changeStatus, takeAction, addNote, fetchStats,
...
export default function CasesPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [c, s] = await Promise.all([listCases(), fetchStats().catch(() => null)])
  ...
  const openCase = async (id) => {
    try { setSelected(await getCase(id)) } catch (e) { setError(e.message) }
  }
  ...
  return (
    <div className="min-h-screen bg-canvas text-content-hi">
      <header className="sticky top-0 z-20 border-b border-edge bg-surface/80 backdrop-blur">
        <div className="mx-auto max-w-[1400px] px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="font-display font-bold text-lg leading-none">Case Management</h1>
              <p className="text-xs text-content-md mt-0.5">Investigate, act, and audit flagged accounts</p>
            </div>
          </div>
      ...
      <main className="mx-auto max-w-[1400px] px-6 py-6">
        {stats && (
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 mb-6">
            ...
            <StatTile label="Reviewing" value={stats.by_status?.reviewing ?? 0} />
            <StatTile label="Action taken" value={stats.by_status?.action_taken ?? 0} />
        ...
        {loading ? (
          <div className="text-content-md text-sm py-20 text-center">Loading cases…</div>
        ) : cases.length === 0 ? (
          <EmptyState />
        ) : (
              ...
              return (
                <div key={col.key} className="panel p-3">
                  <div className="flex items-center justify-between px-1 mb-3">
                    ...
                    <span className="chip chip-neutral text-xs">{items.length}</span>
                  </div>
                  <div className="space-y-2.5">
                    {items.map((c) => <CaseCard key={c.id} c={c} onClick={() => openCase(c.id)} />)}
                    {items.length === 0 && <p className="text-[11px] text-content-lo px-1 py-6 text-center">No cases</p>}
                  </div>
                </div>
              )
            })}
      ...
      {selected && (
        <CaseDrawer c={selected} onClose={() => setSelected(null)} onUpdate={refreshSelected} afterChange={load} />
      )}
    </div>
  )
}
...
function CaseCard({ c, onClick }) {
  return (
    <button onClick={onClick} className="card card-hover w-full text-left p-3.5">
      <div className="flex items-center justify-between">
        <span className="font-mono text-sm font-semibold">{c.account}</span>
        <span className={`chip text-[11px] ${TIER_CLS[c.tier] || ''}`}>{c.tier}</span>
      </div>
      <div className="mt-2 flex items-center justify-between">
        <span className="text-xs text-content-md">Risk</span>
        <span className="num text-sm font-bold">{Math.round(c.risk_score)}</span>
      </div>
      {c.recommended_action && (
        <p className="mt-2 text-[11px] text-content-lo">
          Recommended: <span className="text-brand-600 dark:text-brand-300 font-medium">{c.recommended_action}</span>
        </p>
      )}
      {c.flag_count > 1 && <p className="mt-1 text-[11px] text-content-lo">Flagged {c.flag_count}×</p>}
    </button>
  )
}
  ...
  return (
    <div className="card p-12 text-center">
      ...
      <p className="text-sm text-content-md mt-1 max-w-md mx-auto">
        Run an analysis on the dashboard — flagged accounts will appear here as cases ready to
        investigate, action, and audit.
      </p>
      <Link to="/dashboard" className="btn-brand mt-5 inline-flex">Go to dashboard</Link>
    </div>
  )
}
...
function CaseDrawer({ c, onClose, onUpdate, afterChange }) {
  ...
  const wrap = async (fn) => {
    setBusy(true)
    try { const updated = await fn(); onUpdate(updated); afterChange() }
      ...
      <aside className="relative w-full max-w-md h-full bg-surface border-l border-edge overflow-y-auto scroll-thin">
        ...
        <div className="p-5 space-y-6">
          <div className="grid grid-cols-3 gap-3">
            ...
            <MiniStat label="Prediction" value={c.prediction === 1 ? 'Mule' : 'Legit'} />
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {['new', 'reviewing', 'action_taken', 'closed'].map((s) => (
                <button key={s} disabled={busy || c.status === s}
          ...
          <div>
            <p className="text-xs font-semibold text-content-md mb-2">
              Prevention action
              {c.recommended_action && <span className="text-content-lo font-normal"> · recommended: {c.recommended_action}</span>}
            </p>
            ...
            {c.last_action && <p className="text-[11px] text-content-lo mt-2">Last action: {c.last_action}</p>}
          </div>

          <div>
            <p className="text-xs font-semibold text-content-md mb-2">Add note</p>
            <div className="flex gap-2">
              ...
              <button disabled={busy || !note.trim()}
                onClick={() => wrap(async () => { const u = await addNote(c.id, note); setNote(''); return u })}
                className="btn-brand px-3"><FileText className="h-4 w-4" /></button>
            </div>
          </div>

          <div>
            <p className="text-xs font-semibold text-content-md mb-3 flex items-center gap-1.5">
              <Clock className="h-3.5 w-3.5" /> Audit trail <span className="text-content-lo font-normal">· append-only</span>
            </p>
            <ol className="relative border-l border-edge pl-4 space-y-4">
              {(c.audit || []).map((a) => (
                <li key={a.id} className="relative">
                  ...
                  <p className="text-[11px] text-content-lo mt-0.5">{a.event_type} · {a.actor} · {fmt(a.created_at)}</p>
                </li>
              ))}
              {(!c.audit || c.audit.length === 0) && <li className="text-[11px] text-content-lo">No events yet.</li>}
            </ol>
          </div>
        </div>
      </aside>
